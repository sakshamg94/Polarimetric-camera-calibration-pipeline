...
load('ZOOMED_mean_2DFFT_boils_3fps_25Pump_25cmH.mat');
...
title({'Water jet 25fps;', 'integral normalized $$\left <\log^2\left |  FFT\left(\tan^{-1}\frac{S_{spanwise}}{S_{streamwise}}\right)\right |\right >$$'},...
    'interpreter','latex', 'fontsize',11)

% plot the +/- 45 degree lines
subplot(2,1,2);
plus45 = zeros(mx);
k_mag = zeros(mx);
minus45 = zeros(mx);
for t = 1:1:mx
    k_mag(t) = sqrt(kx(t)^2 + ky(t)^2);
    plus45(t) = I_zoom(t,t);
    minus45(t) = I_zoom(mx-t+1,t);
end
loglog(k_mag, plus45, k_mag, minus45)
legend('+ 45deg', '- 45deg','interpreter','latex', 'fontsize',10)
title('   fall off along +/- 45 degree lines','interpreter','latex', 'fontsize',12)
grid on
axis square
